...
def computeMove(move, turn, whitePieces, blackPieces, board):
    ...
    if piece == 'R':
        ...
        loc = [0,0]
        legal,loc = moveKing(board, colLoc, rowLoc, turn)
        #make sure not in line with King
        if legal:
            
            legal,blank = moveKing(board, colLoc, rowLoc, not turn)
            if legal:
                legal = False
                return legal
            else:
    ...
    if legal:
        ...
        ch = check(board,not turn)#not turn because after play move, see if current move puts opponent in check
        if ch:
            print("Check!")
            print('')
